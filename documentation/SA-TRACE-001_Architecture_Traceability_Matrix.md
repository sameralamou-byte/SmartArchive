...
| Dependencies | [SA-ARCH-000](SA-ARCH-000_Master_Architecture.md), [SA-ARCH-011](SA-ARCH-011_Capability_Map.md), [SA-ARCH-012](SA-ARCH-012_Domain_Model.md), all `ADR-*` including **Approved** [ADR-009](adr/ADR-009-account-tenant-family-entitlement.md), [ADR-010](adr/ADR-010-esa-organizational-hierarchy-and-boundary-model.md), [ADR-011](adr/ADR-011-device-and-intake-governance.md), [ADR-012](adr/ADR-012-ai-permission-inheritance.md), [ADR-013](adr/ADR-013-integrated-standalone-esa-equal-modes.md) |
...
| Administration | #8 | [ADR-010](adr/ADR-010-esa-organizational-hierarchy-and-boundary-model.md) (**Approved** — Scoped Administration; existing admin surfaces predate this ADR and are the traced part below) | `routers/v1/users.py` | *(none found dedicated to this router)* | `ci.yml` → `backend` | Partially traced (existing Tenant-wide admin surfaces); Scoped Administration extension **not yet traceable** — Approved architecture, no implementation |
...
| AI | #7 | ADR-006 + ADR-007 (AI Provider Contract) — **Approved**; [ADR-012](adr/ADR-012-ai-permission-inheritance.md) (**Approved** — AI Permission Inheritance, mandatory authorization precondition for any future implementation); AI Gateway orchestration itself still Wave 1 (`B1`) | `models/ai_job.py` (schema only) | none | none | Not yet traceable — contract Approved, no implementation |
...
| Workflow | #7, #8 | ADR-006 + ADR-007 (Workflow Contract, added on review) — **Approved**; [ADR-012](adr/ADR-012-ai-permission-inheritance.md) §21 (**Approved** — Governed Automation / Consequential-Action distinction); orchestration itself still undesigned | none (only `DocumentStatus` lifecycle enum exists — not a workflow engine) | none | none | Not yet traceable — contract Approved, no implementation |
...
| Connectors | #5 | ADR-006 (Platform Extension Model) + ADR-007 (Connector Contract) — **Approved**; [ADR-013](adr/ADR-013-integrated-standalone-esa-equal-modes.md) (**Approved** — Connector role unchanged across operating-mode configurations); detailed connector semantics still `B3` | `deployment_mode` field on `Organization` only | none | none | Not yet traceable — extension model + contract Approved, no implementation |
| Intake *(new row)* | #8 | ADR-006 (Intake Extension Category) + ADR-007 (Intake Contract) — **Approved**; [ADR-011](adr/ADR-011-device-and-intake-governance.md) (**Approved** — Device & Intake Governance) | none | none | none | Not yet traceable — architecture Approved, no implementation |
...
**Correction on review:** this row (Automation) was missing from the original version of this matrix — [SA-ARCH-011](SA-ARCH-011_Capability_Map.md) listed 21 capabilities, but Section B originally covered only 20. Added in v1.1. Wave B (v1.4) adds Entitlement and Trust & Abuse; Identity and Audit rows are extended. ESA ADR-010–013 synchronization (v1.6) adds the **Intake** row. Capability count in Section B is now **24**.
...
Of the **24** capabilities in Section B: **3 fully traced** (Authentication, Authorization, Storage), **4 partially traced** (Identity, Audit, Monitoring, Administration), **17 not yet traceable** (previous 16 plus **Intake**). Entitlement and Trust & Abuse cite **Approved ADR-009** and Draft SA-ARCH-014 — architecture documented, **not implemented**. Intake cites **Approved ADR-006/007/011** — architecture documented, **not implemented**. This matrix does not treat Approved architecture as implementation.
...
| 1.6 | Synchronization following the Founder-approved ESA ADR-010–013 Architecture Draft Package (2026-08-30), per the Founder-approved Synchronization Impact Audit. Added ADR-012 citation to **AI**; ADR-012 §21 citation to **Workflow**; ADR-013 citation to **Connectors**; ADR-010 citation to **Administration** (existing Tenant-wide surfaces remain Partially traced; the new Scoped Administration extension is separately marked Not yet traceable within the same row, not conflated with it). Added new **Intake** row (Not yet traceable). Identity and Section C rows: reviewed, no genuine ADR-010–013 impact found, left unchanged. Rollup recount: capability count in Section B corrected from 23 to **24**; not-yet-traceable count from 16 to **17**. No traceability-status upgrade for any capability — Approved architecture is not treated as implementation, per this matrix's own standing rule. |
